...
<div class="flex">
    ...
    @if($todo->completed)
    <input wire:click="toggle({{$todo->id}})"  class="mr-4" type="checkbox"  checked>
   @else
   <input wire:click="toggle({{$todo->id}})"   class="mr-4" type="checkbox">
@endif
...
@else
        ...
        @endif
            ...
            <div class="flex items-center space-x-2">
                ...
                <button wire:click.prevent="delete({{$todo->id}})" class="text-sm text-red-500 font-semibold rounded hover:text-teal-800 mr-1">
                    @if(session('message'))
                    <span class="text-green-500 text-xs">{{session('message')}}</span>
                    @endif
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
